#!/usr/bin/env python3
"""
Enhanced Utilities for Unified Sentiment Analysis Pipeline - WITH INFERENCE MODE SUPPORT
...
- **NEW: Full inference mode support for datasets without labels**
...
def analyze_csv(file_path: str, mode: str = "deep") -> Dict[str, Any]:
    ...
    try:
        ...
        is_valid, issues, inference_mode = validate_csv_for_analysis(df)
        ...
        if inference_mode or label_col is None:
            logger.info("🔎 CSV Analysis in INFERENCE MODE: No valid labels detected")
            df_clean['sentiment_label'] = 'unknown'
            
            # For inference mode, still generate insights about the text data
        ...
        metadata = {
            ...
            "analysis_mode": mode,
            "has_labels": label_col is not None and not inference_mode,
            "inference_mode": inference_mode,
            "predicted_labels": bool(predicted_labels) if not inference_mode else False,
            "timestamp": datetime.now().isoformat()
        }
        
        # Add sentiment distribution if available
        if 'sentiment_label' in df_clean.columns and not inference_mode:
        ...
        file_paths = {}
        try:
            analysis_type = f"csv_analysis_{mode}"
            if inference_mode:
                analysis_type += "_inference"
            
            session_dir = create_timestamped_session_dir(analysis_type)
            ...
            if 'sentiment_label' in df_clean.columns and (predicted_labels or not inference_mode):
        ...
        mode_str = "INFERENCE" if inference_mode else "TRAINING"
        logger.info(f"CSV analysis completed in {mode_str} mode. Processed {len(df_clean)} samples, found {len(top_words)} keywords, {len(top_phrases)} phrases, {len(topics)} topics")
        return results
...
def validate_and_preprocess_csv(csv_path: str, output_dir: Optional[str] = None) -> Tuple[bool, str, Dict]:
    ...
    try:
        ...
        if label_col is not None and not inference_mode:
            ...
            # Add placeholder labels for inference mode
            df_processed['label'] = -1
            logger.info("🔎 INFERENCE MODE: Added placeholder labels (-1) for all samples")
        ...
        # Create train/val/test splits if NOT in inference mode
        if not inference_mode and 'label' in df_processed.columns:
            ...
            # Inference mode or no labels, use all data as test set
            ...
            if inference_mode:
                validation_info['note'] = 'Inference mode - all data treated as test set with placeholder labels'
            else:
        ...
        # Save inference file if in inference mode
        if inference_mode:
            inference_path = output_dir / "inference.csv"
            test_df.to_csv(inference_path, index=False)
            validation_info['inference_file'] = str(inference_path)
        ...
        mode_str = "INFERENCE" if inference_mode else "TRAINING"
        logger.info(f"Preprocessing completed in {mode_str} mode. Files saved to: {output_dir}")
...
def run_embedding_generation(processed_dir: str, embeddings_dir: str) -> Tuple[bool, Dict]:
    ...
    Enhanced to handle inference mode.
    ...
    try:
        ...
        # Check if we're in inference mode by looking at the data
        ...
        if test_data_path.exists():
            ...
            if 'label' in test_df.columns:
                # Check if all labels are -1 (inference mode)
                unique_labels = test_df['label'].unique()
                if len(unique_labels) == 1 and unique_labels[0] == -1:
                    inference_mode = True
                    logger.info("🔎 Detected inference mode from data labels")
        ...
        if result.returncode == 0:
            ...
            if inference_mode:
                # In inference mode, we expect different files
                expected_files = ['X_test.npy', 'embedding_metadata.json']
                # Y files might be empty or contain placeholder values
                optional_files = ['y_test.npy', 'X_train.npy', 'y_train.npy', 'X_val.npy', 'y_val.npy']
            else:
                # Training mode expects all files
            ...
            optional_generated = [f for f in optional_files if (embeddings_path / f).exists()]
            ...
            embedding_info = {
                ...
                'optional_files': optional_generated,
...
def run_model_training(embeddings_dir: str, models_dir: str, fast_mode: bool = True) -> Tuple[bool, Dict]:
    ...
    Enhanced to handle inference mode (skip training if no valid labels).
    ...
    try:
        ...
        X_train_path = embeddings_path / "X_train.npy"
        y_train_path = embeddings_path / "y_train.npy"
        
        if not X_train_path.exists() or not y_train_path.exists():
            inference_mode = True
            logger.info("🔎 No training data found - skipping model training (inference mode)")
            
            training_info['mode'] = 'inference'
            training_info['note'] = 'Skipped training - no training data available (inference mode)'
            
            return True, training_info  # Return success but no training performed
        
        # Check if training data is valid (not just placeholder values)
        try:
            ...
            if len(unique_labels) == 1 and unique_labels[0] == -1:
                inference_mode = True
                logger.info("🔎 Training labels are placeholders (-1) - skipping model training (inference mode)")
                
                training_info['mode'] = 'inference'
                training_info['note'] = 'Skipped training - placeholder labels detected (inference mode)'
                
                return True, training_info
        except Exception as e:
            logger.warning(f"Could not check training labels: {e}")
        
        # Proceed with normal training
        logger.info("🎯 TRAINING MODE: Valid training data detected")
        training_info['mode'] = 'training'
...
def run_prediction_and_evaluation(models_dir: str, processed_dir: str, results_dir: str) -> Tuple[bool, Dict]:
    ...
    try:
        ...
        if 'label' in test_df.columns:
            ...
            if len(unique_labels) == 1 and unique_labels[0] == -1:
                inference_mode = True
                logger.info("🔎 INFERENCE MODE: Placeholder labels detected in test data")
        ...
        # Add inference mode flag if applicable
        if inference_mode:
            cmd.append("--inference-mode")
        
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=300)
        
        if result.returncode == 0:
            mode_str = "INFERENCE" if inference_mode else "TRAINING"
            logger.info(f"Evaluation completed successfully in {mode_str} mode")
...
def auto_embed_and_predict(file_path: str, fast_mode: bool = True, 
    ...
    Enhanced to fully support inference mode for datasets without labels.
    ...
    pipeline_results = {
        ...
        'fast_mode': fast_mode,
        'inference_mode': False,  # Will be updated during processing
        'steps': {}
    }
    ...
    try:
        ...
        # Check if we're in inference mode
        inference_mode = preprocessing_info.get('inference_mode', False)
        pipeline_results['inference_mode'] = inference_mode
        
        mode_str = "INFERENCE" if inference_mode else "TRAINING"
        logger.info(f"🔍 Detected {mode_str} MODE")
        ...
        if not emb_success:
            logger.warning("Embedding generation failed, pipeline cannot continue without embeddings")
            pipeline_results['overall_success'] = False
            pipeline_results['error'] = 'Embedding generation failed'
            return pipeline_results
        
        # Step 3: Train models (skip in inference mode)
        logger.info("Step 3: Model training...")
        models_dir = session_dir / "models"
        
        if inference_mode:
            logger.info("🔎 INFERENCE MODE: Skipping model training")
            pipeline_results['steps']['training'] = {
                'success': True,
                'skipped': True,
                'reason': 'Inference mode - no training data available',
                'info': {'mode': 'inference', 'note': 'Model training skipped for inference mode'}
            }
        else:
        ...
        # For inference mode, focus on text analysis rather than model evaluation
        if inference_mode:
            logger.info("🔎 INFERENCE MODE: Generating text analysis insights")
            
            # Generate text analysis for the dataset
            if test_data_path.exists():
                test_df = pd.read_csv(test_data_path)
                ...
                # Generate insights about the text data
                insights = generate_inference_insights(test_df, all_text)
                pipeline_results['insights'] = insights
                
                # Save insights to file
                insights_path = session_dir / "inference_insights.txt"
                with open(insights_path, 'w', encoding='utf-8') as f:
                    f.write('\n'.join(insights))
                
                pipeline_results['inference_analysis'] = {
                    'total_samples': len(test_df),
                    'ready_for_prediction': True,
                    'text_statistics': {
                        'total_characters': len(all_text),
                        'avg_text_length': test_df['text'].str.len().mean(),
                        'unique_words': len(set(all_text.lower().split()))
                    }
                }
        else:
            # Training mode - try to load trained models and make predictions for GUI
            try:
                predictor = SentimentPredictor()
            ...
            # Generate intelligent insights for training mode
            if test_data_path.exists():
                test_df = pd.read_csv(test_data_path)
        ...
        critical_steps = ['preprocessing', 'embedding']
        
        # In inference mode, training is optional
        if not inference_mode:
            critical_steps.append('training')
        
        optional_steps = ['evaluation']
        ...
        mode_str = "INFERENCE" if inference_mode else "TRAINING"
        logger.info(f"Pipeline completed in {mode_str} mode. Results saved to: {session_dir}")
        return pipeline_results
...
def generate_inference_insights(df: pd.DataFrame, all_text: str) -> List[str]:
    """
    Generate insights specific to inference mode datasets.
    
    Args:
        df: DataFrame with the inference data
        all_text: Combined text from all samples
    ...
    try:
        # Dataset insights
        n_samples = len(df)
        insights.append(f"🔎 **Inference Mode Analysis**: Processed {n_samples:,} samples for sentiment prediction")
        ...
        if 'text' in df.columns:
            ...
            if short_texts > n_samples * 0.1:
                insights.append(f"⚠️ **Data Quality Alert**: {short_texts} texts ({short_texts/n_samples*100:.1f}%) are very short (<20 chars) - may affect prediction accuracy")
        ...
        insights.append(f"📚 **Vocabulary Analysis**: {len(unique_words):,} unique words, diversity ratio: {vocab_diversity:.3f}")
        
        if vocab_diversity > 0.5:
            insights.append("🎯 **Vocabulary Quality**: High vocabulary diversity - good for sentiment analysis")
        elif vocab_diversity > 0.3:
            insights.append("📊 **Vocabulary Quality**: Moderate vocabulary diversity - acceptable for analysis")
        else:
            insights.append("⚠️ **Vocabulary Quality**: Low vocabulary diversity - may limit prediction accuracy")
        
        # Text distribution insights
        sentence_counts = [len(sent_tokenize(text)) for text in df['text'].head(100)]  # Sample for performance
        avg_sentences = np.mean(sentence_counts) if sentence_counts else 0
        
        insights.append(f"📄 **Text Structure**: Average {avg_sentences:.1f} sentences per text")
        
        # Readiness assessment
        insights.append("🚀 **Pipeline Status**: Dataset preprocessed and ready for sentiment prediction")
        insights.append("🔮 **Next Steps**: Embeddings will be generated and models will predict sentiment for all samples")
        ...
        if null_count == 0 and empty_count == 0:
            insights.append("✅ **Data Preparation**: All texts are valid and ready for processing")
        else:
            total_issues = null_count + empty_count
            insights.append(f"🔧 **Data Issues**: {total_issues} problematic entries detected and handled")
        
        # Expected output format
        insights.append("📊 **Expected Output**: Sentiment predictions (positive/negative) with confidence scores for each text")
        insights.append("💾 **Results**: Predictions will be saved as CSV with original text and predicted sentiment")
        
    except Exception as e:
        logger.error(f"Error generating inference insights: {e}")
...
def validate_csv_for_analysis(df: pd.DataFrame) -> Tuple[bool, List[str], bool]:
    """
    Validate CSV data for sentiment analysis.
    Enhanced to detect inference mode.
    
    Args:
        df: DataFrame to validate
    
    Returns:
        Tuple of (is_valid, list_of_issues, inference_mode)
    """
    issues = []
    inference_mode = False
    ...
    # Check label column if present and determine inference mode
    label_columns = ['label', 'sentiment', 'class', 'target', 'rating']
    ...
    for col in label_columns:
        if col in df.columns:
            ...
            if len(unique_labels) == 0:
                ...
                # Single label - likely inference mode
                inference_mode = True
                break
            
            # Check for valid label formats
            valid_formats = [
            ...
            if any(set(unique_labels).issubset(valid_format) for valid_format in valid_formats):
    ...
    # Final inference mode determination
    if not has_valid_labels:
        inference_mode = True
        logger.info("🔎 INFERENCE MODE: No valid labels detected")
    ...
    return is_valid, issues, inference_mode
...
def create_analysis_report(results: Dict[str, Any]) -> str:
    ...
    # Check if this is inference mode
    inference_mode = results.get('inference_mode', False)
    mode_str = "INFERENCE" if inference_mode else "TRAINING"
    
    report_lines.append(f"SENTIMENT ANALYSIS REPORT - {mode_str} MODE")
    report_lines.append("=" * 60)
    ...
    if inference_mode:
        report_lines.append(f"  Mode: INFERENCE (Prediction Only)")
    else:
        report_lines.append(f"  Mode: TRAINING (With Labels)")
    ...
    if 'statistics' in results:
        ...
        if 'label_stats' in stats and not inference_mode:
    ...
    # Metrics (only for training mode)
    if 'metrics' in results and not inference_mode:
        report_lines.append("")
    ...
    if 'prediction_summary' in results:
        report_lines.append("")
        if inference_mode:
            report_lines.append("INFERENCE PREDICTIONS:")
        else:
    ...
    # Inference mode specific information
    if inference_mode:
        report_lines.append("")
        report_lines.append("INFERENCE MODE NOTES:")
        report_lines.append("  • No ground truth labels available for evaluation")
        report_lines.append("  • Predictions generated using pre-trained models")
        report_lines.append("  • Results ready for export and further analysis")
...
def compare_analysis_results(results1: Dict[str, Any], results2: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Enhanced to handle inference mode comparisons.
    ...
    comparison = {
        ...
        'mode1': 'INFERENCE' if results1.get('inference_mode', False) else 'TRAINING',
        'mode2': 'INFERENCE' if results2.get('inference_mode', False) else 'TRAINING',
        'differences': {}
    }
    
    # Compare metrics if both have them (training mode only)
    if ('metrics' in results1 and 'metrics' in results2 and 
        not results1.get('inference_mode', False) and not results2.get('inference_mode', False)):
    ...
    if 'prediction_summary' in results1 and 'prediction_summary' in results2:
        comparison['prediction_differences'] = {}
        
        common_models = set(results1['prediction_summary'].keys()) & set(results2['prediction_summary'].keys())
        for model in common_models:
            summary1 = results1['prediction_summary'][model]
            summary2 = results2['prediction_summary'][model]
            
            ratio_diff = summary2.get('positive_ratio', 0) - summary1.get('positive_ratio', 0)
            
            comparison['prediction_differences'][model] = {
                'positive_ratio_change': ratio_diff,
                'total_predictions_1': summary1.get('total_predictions', 0),
                'total_predictions_2': summary2.get('total_predictions', 0)
            }
